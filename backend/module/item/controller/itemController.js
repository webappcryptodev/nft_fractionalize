...
exports.publish = function(req,res) {
    ...
    items.findOne({_id:req.body.item_id, author_id: req.decoded.user_id, status:"inactive"}).populate('collection_id').exec(function (err, item) {
        ...
        userController.getUserInfoByID(req.decoded.user_id, async function(err,user){
          try {
              var scCollection = new web3.eth.Contract(config.collection_abi, item.collection_id.contract_address);

              var encoded = scCollection.methods.mintNFTs(1).encodeABI()
              var tx = {
              ...
              let transactionHash = await web3.eth.sendSignedTransaction(signed.rawTransaction, (error, result) => {
                if(!error){
                  console.log(result);
                    ...
                    res.json({
                        status: false,
                        message: error.toString().split('ERROR: ').pop().replace(/\n|\r/g, "")
                    });
                    return

                }
              });
              // Get Status Block from transactionHash...

              var status_block = "Failed"
            ...
            userController.getUserInfoByID(req.decoded.user_id,function(err,sender){
                this.checkbalance(sender.public_key,item,function(has_balance) {
                    ...
                    this.transferAdminComission(item, function(error, comission){
                        this.transferBalance(sender,receiver, item, comission, async function(is_transferred){
                            if(!has_balance) {
                                res.json({
                                    status: false,
                            ...
                            try {
                                ...
                                var encoded = scCollection.methods.transferFrom(sender.public_key, receiver.public_key, item.token_id).encodeABI()
                                var tx = {
                                    gasLimit: web3.utils.toHex(2000000),
                                    from: user.public_key,
                                    data: encoded
                                }
                                let signed = await web3.eth.accounts.signTransaction(tx, receiver.private_key);
                                let transactionHash = await web3.eth.sendSignedTransaction(signed.rawTransaction, (error, result) => {
                                  if(!error){
                                    console.log(result);
                                  }else{
...
exports.sendETH = function(req,res) {
    ...
    userController.getUserInfoByID(req.decoded.user_id, function(err,user){
        web3.eth.getBalance(user.public_key).then(async (balance) => {
            ...
            if(eth>req.body.amount) {
              var tx = {
                  ...
                  from: user.public_key,
                  to: req.body.eth_address,
                  value: web3.utils.toWei(req.body.amount, 'ether')
              }
              let signed = await web3.eth.accounts.signTransaction(tx, user.private_key);
              web3.eth.sendSignedTransaction(signed.rawTransaction, (error, result) => {
                if(!error){
...
transferAdminComission = function(item, callback) {
    options.findOne({name:"admin_commission"}, function (err, option) {
        ...
        userController.getUserInfoByID(item.current_owner,function(err,sender){
            users.findOne({role:1}).exec(async function(err,receiver){
                ...
                var tx = {
                    ...
                    value: web3.utils.toWei(commission, 'ether')
                }
                let signed = await web3.eth.accounts.signTransaction(tx, sender.private_key);
                web3.eth.sendSignedTransaction(signed.rawTransaction, (error, result) => {
                  if(!error){
                    console.log("Tx : " + result);
                  }else{
 ...
 */
transferBalance = async function(sender, receiver, item, commission, callback) {
    ...
    if(author_id == receiver_id) {
      ...
      var priceWithoutComission = item.price - commission;
       ...
       userController.getUserInfoByID(item.author_id, async function(err, author){
        console.log("royalties going to send ",royalty);
        var tx = {
            gasLimit: web3.utils.toHex(6200000),
            from: sender.public_key,
            to: receiver.public_key,
            value: web3.utils.toWei(royalty, 'ether')
        }
        let signed = await web3.eth.accounts.signTransaction(tx, sender.private_key);
        web3.eth.sendSignedTransaction(signed.rawTransaction, (error, result) => {
          if(!error){
